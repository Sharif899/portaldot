...
export default function Tokenize() {
  ...
  // ── Mint: save to Supabase instead of localStorage ───────────
  const handleMint = async () => {
    ...
    const newAsset = {
      ...
      value_usd:           Number(assetValue),
      fractions:           Number(fractions),
      fractions_available: Number(fractions),
      price_per_fraction:  0.35,
      ipfs_cid:            ipfsCid,
      is_verified:         false,
      status:              "Active",
      location:            location,
    ...
    const { error } = await supabase.from("assetdot").insert([newAsset]);

    if (error) {
      console.error("Supabase insert error:", error);
      setMintError("Failed to save asset. Please try again.");
      setMinting(false);
      return;
    }
              ...
              <div key={s.id} style={{ display: "flex", alignItems: "center", flex: 1 }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px" }}>
                  ...
                  <span style={{
                    ...
                    color: step === s.id ? "var(--brand)" : "var(--text-muted)", whiteSpace: "nowrap",
                  }}>
                    {s.label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
                  <div style={{
                    ...
                    marginBottom: "22px", transition: "background 0.3s ease",
                  }} />
                )}
              </div>
            ))}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "24px" }}>
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. LIAB" className="input"
                    />
            ...
            <div style={{ maxWidth: "600px" }}>
              <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "16px", padding: "24px" }}>
                ...
                <p style={{ fontSize: "13px", color: "var(--text-secondary)", margin: "0 0 20px" }}>
                  Upload your asset's legal document. It will be stored encrypted on IPFS.
                </p>
          ...
          {step === 4 && (
            <div style={{ maxWidth: "600px" }}>
              <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "16px", padding: "24px" }}>
                ...
                {mintError && (
                  <p style={{ fontSize: "13px", color: "var(--accent-coral)", margin: "12px 0 0", textAlign: "center" }}>
                    {mintError}
                  </p>
                )}
      ...
      <Modal isOpen={showSuccess} onClose={() => setShowSuccess(false)} title="🎉 Asset Tokenized!" size="sm">
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ fontSize: "13px", color: "var(--text-secondary)", margin: "0 0 16px" }}>
            Your asset has been deployed and is now visible on the marketplace.
          </p>
          <div style={{
